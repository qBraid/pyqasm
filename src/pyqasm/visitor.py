...
class QasmVisitor:
    ...
    def _visit_delay_statement(
        ...
        Args:
            statement (openqasm3.ast.DelayInstruction): The DelayInstruction statement to visit.
        Returns:
            list[openqasm3.ast.Statement]: The list of statements generated by the DelayInstruction.
        """
        ...
        if not delay_qubit_bits:
            ...
            for reg_name, reg_size in self._global_qreg_size_map.items():
                for reg in range(reg_size):
                    delay_on_all_qubits.append(
                        qasm3_ast.IndexedIdentifier(
                            name=qasm3_ast.Identifier(reg_name),
                            indices=[[qasm3_ast.IntegerLiteral(reg)]],
                        )
                    )
            statement.qubits = delay_on_all_qubits  # type: ignore[assignment]
        else:
    ...
    def _visit_box_statement(self, statement: qasm3_ast.Box) -> list[qasm3_ast.Statement]:
        ...
        Args:
            statement (openqasm3.ast.Box): The Box statement node to visit.
        Returns:
            list[openqasm3.ast.Statement]: The list of statements generated by the Box statement.
        """
        statements = []
        ...
        if _box_time_var and self._total_delay_duration_in_box > box_duration_val:
