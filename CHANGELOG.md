...
- Fixed subroutines returning `bit` / `bit[n]`: `bit x = f();` raised `AttributeError: 'NoneType' object has no attribute 'value'`, or emitted an unresolved call. `return measure q;` now works, measuring the caller's qubit into a temporary bit. A declared/returned type mismatch raises `ValidationError` naming both types. ([#387](https://github.com/qBraid/pyqasm/issues/387))
